 ...
 *
 * The native module is loaded lazily so the app keeps running in Expo
 * Go (where the native module is missing) and falls back to a
 * Free-plan mock when no API key is configured. We type the SDK with
 * a local minimal surface so this file type-checks without the
...
export async function purchasePackageByIdentifier(
  ...
  if (!offering) {
    return { status: 'error', message: 'no-offering' };
  }
  const pkg =
  ...
  if (!pkg) {
    return { status: 'error', message: 'no-package' };
  }
  try {

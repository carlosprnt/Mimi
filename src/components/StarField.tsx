...
const buildStars = (count: number, seed: number, heightFraction: number): Star[] => {
  ...
  for (let i = 0; i < count; i++) {
    const sizeRoll = rand();
    const size = sizeRoll < 0.82 ? 1 : sizeRoll < 0.96 ? 1.8 : 2.6;
    const baseOpacity = 0.35 + rand() * 0.45;
    stars.push({
      x: rand(),
...
export const StarField: React.FC<StarFieldProps> = ({
  ...
  React.useEffect(() => {
    ...
    loop(phase0, 2600, 0.55, 0);
    loop(phase1, 3400, 0.4, 900);
    loop(phase2, 4200, 0.65, 1800);
